...
if __name__ == '__main__':
    ...
    if os.path.exists(overall_summary_path):
        ...
        # if model and non-whitespace-only promopt arguments provided (FEVER)
        if len(sys.argv) == 3 and sys.argv[2].strip():
            prompt = f"Considering the following {doctype}, based on its abstract and summaries of each section:\n{subcontent}\nPlease answer the following prompt:{sys.argv[2]}{output_language_prompt}"
